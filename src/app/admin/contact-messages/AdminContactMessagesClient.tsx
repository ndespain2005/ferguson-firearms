...
import { updateContactMessage, type ContactStatus } from "./actions";
...
const STATUSES: ContactStatus[] = ["New", "In Progress", "Closed"];
...
export default function AdminContactMessagesClient({ initialRows }: { initialRows: Row[] }) {
  ...
  async function save(id: string) {
    ...
    if (!r) return;
    const status = (STATUSES.includes((r.status as any) ?? "New") ? (r.status as any) : "New") as ContactStatus;

    setSavingId(id);
    setToast(null);
    try {
      await updateContactMessage({ id, status, internalNotes: r.internal_notes ?? "" });
              ...
              <tr key={r.id} className="border-t border-border align-top">
                ...
                <td className="py-3 pr-4">
                  ...
                  <div className="mt-2 text-xs">
                    <span className="text-muted">Topic:</span> {r.topic}
                  </div>
                </td>
                <td className="py-3 pr-4">
                  <details>
